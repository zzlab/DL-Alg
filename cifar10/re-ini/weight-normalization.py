...
for i in range(iterations):
  ...
  if (i + 1) % validation_interval == 0:
    accuracy = classification_accuracy(model, validation_X, validation_Y)
    print 'iteration %d accuracy %f' % (i + 1, accuracy)
  '''

  if (i + 1) % checkpoint_interval == 0:
    file_name = 'weight-normalization-checkpoints/iteration-%d' % (i + 1)
    for key, value in storage.items():
      storage[key] = value.asnumpy()
    pickle.dump(storage, open(file_name, 'wb'))
    file_name = 'weight-normalization-checkpoints/parameters-iteration-%d' % (i + 1)
